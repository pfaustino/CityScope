import { NWS_DWML_URL, NWS_JSON_FALLBACK_URL, USGS_QUAKEML_URL } from './publicFeeds.ts'
...
export const SOURCES: SourceRecord[] = [
  ...
  },
  {
    ...
    apiEndpoint: NWS_DWML_URL,
    datasetId: 'LOX/154,51',
    authentication: 'PUBLIC',
    rateLimit: 'Public DWML; be polite',
    updateFrequency: 'daily',
    geographicCoverage: 'Burbank (~34.1808, -118.309)',
    ...
    dataQualityRating: 5,
    legalAccess: 'PUBLIC',
    howToObtain: `Browser GET ${NWS_DWML_URL} (DWML XML, no key). If XML is blocked, fall back to ${NWS_JSON_FALLBACK_URL} and label that JSON fallback. NOAA CDO is a separate keyed archive — do not put that token in the browser.`,
    phase1Priority: 9,
  },
  {
    id: 'usgs-earthquakes',
    ...
    apiEndpoint: USGS_QUAKEML_URL,
    datasetId: 'fdsnws-event',
    authentication: 'PUBLIC',
    rateLimit: 'USGS FDSN; keep queries bounded',
    updateFrequency: 'daily',
    geographicCoverage: '40 km radius from Burbank center',
    historicalCoverage: '2026 YTD in the live browser query (starttime=2026-01-01); catalog itself is longer',
    fieldsAvailable: ['magnitude', 'place', 'time', 'coordinates', 'depth', 'event url'],
    lastSuccessfulRetrieval: '2026-08-15',
    lastModified: '2026-08-15',
    dataQualityRating: 5,
    legalAccess: 'PUBLIC',
    howToObtain: `Browser GET ${USGS_QUAKEML_URL} (QuakeML XML, no key). Filter is 40 km from Burbank center, M≥2.5, 2026-01-01 through request time (2026 YTD). Parse in the client.`,
    phase1Priority: 10,
  },
  {
